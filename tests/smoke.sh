#!/bin/sh
# smoke tests for mote - no network, no API key needed (echo transport).
set -e
cd "$(dirname "$0")/.."

make mote examples/hello_plugin.so >/dev/null 2>&1

echo "== 1: agent loop runs end-to-end (echo transport)"
OUT=$(./mote -p hi --transport echo)
